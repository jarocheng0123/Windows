...
>nul findstr /v "$" "%~nx0" && (
echo:
echo 错误：脚本可能存在LF换行符问题，或者脚本末尾缺少空行。
echo:
...
echo 该项目仅支持用于 Windows 7/8/8.1/10/11 以及它们对应的服务器端版本.
goto done2
)
...
echo 如果你对Powershell施加了限制，那么撤销这些更改。.
echo:
echo 查看此页面获取帮助. %mas%IAS-Help#troubleshoot
goto done2
)
...
%eline%
echo 此脚本需要管理员权限.
echo 要做到这一点，请右键单击此脚本，然后选择“以管理员身份运行”。
goto done2
)
...
echo WMI 无法正常工作。中止...
echo:
echo 查看此页面寻求帮助. %mas%IAS-Help#troubleshoot
goto done2
)
...
echo:
echo:
call :_color2 %_White% "             " %_Green% "创建者 Piash"
...
echo:            ___________________________________________________
echo:         
call :_color2 %_White% "             " %_Green% "在键盘中输入一个菜单选项 [1,2,3,4,5,0]"
choice /C:123450 /N
set _erl=%errorlevel%
...
echo:
echo 正在创建 CLSID 注册表项的备份 %SystemRoot%\Temp
...
echo %line%
echo:
echo      部分用户的激活功能无法正常使用，并且IDM可能会显示虚假的序列号提示屏幕.
echo:
...
echo 你可以从以下地方下载它  https://www.internetdownloadmanager.com/download.html
goto done
)
...
call :_color %Gray% "对internetdownloadmanager.com执行的Ping命令失败"
echo:
)
...
echo:
echo 正在创建以下内容中CLSID注册表项的备份 %SystemRoot%\Temp
...
echo:
call :_color %Gray% "如果出现假序列号界面，请改用“冻结试用”选项。"
) else (
call :_color %Green% "IDM 30天试用期已成功终身冻结。"
echo:
call :_color %Gray% "如果IDM弹出注册提示框，请重新安装IDM。"
)
